package com.shindoclient.shindo.injection.mixin.minecraft.client.renderer;

import com.shindoclient.shindo.management.event.impl.EventFireOverlay;
import com.shindoclient.shindo.management.event.impl.EventRenderItemInFirstPerson;
...
public class MixinItemRenderer {
    ...
    public float modifyTransformItem(float original, float partialTicks) {
        ...
        return mod.isToggled() && blockHitSetting != null && blockHitSetting.isToggled() ? abstractClientPlayer.getSwingProgress(partialTicks) : original;
    }
    ...
    public void oldRod(ItemRenderer instance, EntityLivingBase entityIn, ItemStack heldStack, ItemCameraTransforms.TransformType transform) {
        ...
        AnimationsMod mod = AnimationsMod.instance;
        BooleanSetting rodSetting = mod.getRodSetting();

        if (mod.isToggled() && rodSetting != null && rodSetting.isToggled() && !mc.getRenderItem().shouldRenderItemIn3D(heldStack)) {
    ...
    private void preUpdateEquippedItem(CallbackInfo ci) {
        ...
        AnimationsMod mod = AnimationsMod.instance;
        BooleanSetting itemSwitchSetting = mod.getItemSwitchSetting();

        if (mod.isToggled() && itemSwitchSetting != null && itemSwitchSetting.isToggled()) {
            ci.cancel();
