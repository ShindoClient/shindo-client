...
import me.miki.shindo.ui.comp.mods.CompSlider;
...
public class SpotifyCategory extends Category implements MusicManager.TrackInfoCallback {
    ...
    public SpotifyCategory(GuiModMenu parent) {
        super(parent, TranslateText.SPOTIFY, LegacyIcon.SPOTIFY, true, true);
    ...
    private void drawSetupScreen(NanoVGManager nvg, int mouseX, int mouseY, ColorPalette palette, AccentColor accentColor) {
        ...
        if (setupError) {
            Shindo.getInstance().getNotificationManager().post(TranslateText.MUSIC, TranslateText.SPOTIFY_INVALID_CREDENTIALS, NotificationType.ERROR);
            setupError = false;
        }
    }
    ...
    private String[] wrapText(NanoVGManager nvg, String text, float fontSize, me.miki.shindo.management.nanovg.font.Font font, float maxWidth) {
