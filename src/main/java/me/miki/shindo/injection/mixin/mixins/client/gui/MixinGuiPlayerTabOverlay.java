    ...
    public int renderShindoIcon(FontRenderer fontRenderer, String text, float x, float y, int color) {
        ...
        if (uuid != null) {
            ...
            if (PresenceTracker.isOnline(String.valueOf(uuid))) {
                ...
                if (RoleManager.hasAtLeast(uuid, Role.STAFF)) iconColor = new Color(178, 2, 2);
                else if (RoleManager.hasAtLeast(uuid, Role.DIAMOND)) iconColor = new Color(2, 194, 172);
                else if (RoleManager.hasAtLeast(uuid, Role.GOLD)) iconColor = new Color(227, 216, 0);
