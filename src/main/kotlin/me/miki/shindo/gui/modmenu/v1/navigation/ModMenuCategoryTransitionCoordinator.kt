package me.miki.shindo.gui.modmenu.v1.navigation
...
class ModMenuCategoryTransitionCoordinator {
    ...
    fun update() {
        ...
        if (progressAnimation.value >= 0.999f) {
            fromCategory = null
            progressAnimation.value = 1f
            state = State.IDLE
        }
    ...
    fun isTransitioning(): Boolean {
        return state == State.SWITCHING && fromCategory != null && progressAnimation.value < 0.999f
    }
    ...
    fun buildRenderLayers(contentWidth: Float): List<RenderLayer> {
        ...
        val leaving = fromCategory
        if (leaving == null) {
            return listOf(RenderLayer(category = active, offsetX = 0f, alpha = 1f))
        }
