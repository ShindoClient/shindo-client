...
class HomeCategory(parent: GuiModMenu) : Category(parent, TranslateText.HOME, LegacyIcon.HOME, false, false) {
    ...
    override fun drawScreen(mouseX: Int, mouseY: Int, partialTicks: Float) {
        ...
        val newsList: List<News> = newsManager.news
        if (newsList.isNotEmpty()) {
            // Rotacao automatica a cada 15 segundos
            ...
            val newsHeight = TOP_CARD_HEIGHT // Mesma altura do changelog
            nvg.drawRoundedRect(leftX, topY, columnWidth, newsHeight, 8f, palette.getBackgroundColor(ColorType.DARK))
            nvg.drawText(TranslateText.NEWS.text, leftX + INNER_PADDING, topY + 15 + INNER_PADDING, palette.getFontColor(ColorType.DARK), 11f, Fonts.SEMIBOLD)
            if (newsList.size > 1) {
                ...
                nvg.drawText(newsPageLabel, leftX + columnWidth - newsPageWidth - INNER_PADDING, newsPageY, ColorUtils.applyAlpha(palette.getFontColor(ColorType.NORMAL), 200), 8f, Fonts.REGULAR)
            }
            ...
            nvg.drawTextBox(currentNews.title, leftX + INNER_PADDING, newsY, textWidth, palette.getFontColor(ColorType.DARK), 10f, Fonts.SEMIBOLD)
            newsY += titleSize + 2
            val subTitleSize = nvg.getTextBoxHeight(currentNews.subTitle, 8.5f, Fonts.MEDIUM, textWidth)
            nvg.drawTextBox(currentNews.subTitle, leftX + INNER_PADDING, newsY, textWidth, palette.getFontColor(ColorType.DARK), 8.5f, Fonts.MEDIUM)
            newsY += subTitleSize + 3
            val bodySize = nvg.getTextBoxHeight(currentNews.body, 8f, Fonts.REGULAR, textWidth)
            nvg.drawTextBox(currentNews.body, leftX + INNER_PADDING, newsY, textWidth, palette.getFontColor(ColorType.DARK), 8f, Fonts.REGULAR)
            ...
            // Shadow
            nvg.drawVerticalGradientRect(leftX + INNER_PADDING, topY + 20, columnWidth - (INNER_PADDING * 2f), 8f, palette.getBackgroundColor(ColorType.DARK), noColour)
            nvg.drawVerticalGradientRect(leftX + INNER_PADDING, topY + newsHeight - 8, columnWidth - (INNER_PADDING * 2f), 8f, noColour, palette.getBackgroundColor(ColorType.DARK))
        }
        ...
        nvg.drawText(TranslateText.CHANGELOG.text, rightX + INNER_PADDING, topY + 15 + INNER_PADDING, palette.getFontColor(ColorType.DARK), 11f, Fonts.SEMIBOLD)
        ...
        nvg.drawRoundedRect(progressBarX, progressBarY, progressBarWidth * progress, progressBarHeight, 1f, currentColor.color1)
        ...
        val blockWidth = iconSize + iconGap + textWidth
        ...
        for (c in changelogManager.changelogs) {
        ...
        if (totalPages > 1) {
            ...
            nvg.drawRoundedRect(changelogProgressX, changelogProgressY, changelogProgressWidth, changelogProgressHeight, 1f, ColorUtils.applyAlpha(palette.getBackgroundColor(ColorType.NORMAL), 150))
            nvg.drawRoundedRect(changelogProgressX, changelogProgressY, changelogProgressWidth * changelogProgress, changelogProgressHeight, 1f, currentColor.color1)
            ...
            nvg.drawText(changelogPageLabel, rightX + columnWidth - changelogPageWidth - INNER_PADDING, changelogPageY, ColorUtils.applyAlpha(palette.getFontColor(ColorType.NORMAL), 200), 8f, Fonts.REGULAR)
        }
        ...
        if (pages.isNotEmpty()) {
            ...
            for (c in page) {
                ...
                nvg.drawCenteredText(c.type.text, iconX + (iconSize / 2f), lineY + ((tbSize / 2f) - 1f), Color.WHITE, 7f, Fonts.LEGACYICON)
                nvg.drawTextBox(c.text, textX, lineY + 3f, textWidth, palette.getFontColor(ColorType.DARK), 8f, Fonts.MEDIUM)
        ...
        // Player & Spotify Card (no espaco que sobrou)
        val playerCardY = bottomY
        val playerCardHeight = BOTTOM_CARD_HEIGHT
        val playerCardX = leftX
        val playerCardWidth = columnWidth

        nvg.drawRoundedRect(playerCardX, playerCardY, playerCardWidth, playerCardHeight, 8f, palette.getBackgroundColor(ColorType.DARK))

        val musicManager: MusicManager? = instance.musicManager
        val mc = Minecraft.getMinecraft()
        val spotifyLinked = musicManager != null && musicManager.isAuthorized

        // Player head e nome
        val headSize = 32f
        val headX = playerCardX + INNER_PADDING
        val headY = playerCardY + INNER_PADDING
        val playerName = if (mc.session != null) mc.session.username else TranslateText.PLAYER.text
        val playerHead = if (mc.thePlayer != null) mc.thePlayer.locationSkin else ResourceLocation("textures/entity/steve.png")
        ...
        if (spotifyLinked && musicManager != null) {
            ...
            val trackPosition = musicManager.currentPosition
            val trackDuration = (musicManager.endTime * 1000).toLong() // getEndTime retorna em segundos

            if (currentTrack != null) {
                // Icone da musica (se couber)
                val iconSize1 = 16f
                val iconX1 = headX
                val iconY = spotifyY
                nvg.drawText(LegacyIcon.MUSIC, iconX1, iconY, palette.getFontColor(ColorType.NORMAL), iconSize, Fonts.LEGACYICON)

                // Nome da musica
                val trackName = currentTrack.name
                val artistName = if (currentTrack.artists != null && currentTrack.artists.isNotEmpty()) currentTrack.artists[0].name else TranslateText.UNKNOWN.text
                val trackNameX = iconX1 + iconSize1 + 4
                val trackNameWidth = playerCardWidth - (INNER_PADDING * 2f)
                ...
                // Previous
                val prevHovered = MouseUtils.isInside(mouseX, mouseY, controlsCenterX - controlSpacing - controlSize / 2f, controlsY, controlSize, controlSize)
                nvg.drawCenteredText(LegacyIcon.BACK, controlsCenterX - controlSpacing, controlsY + 2, ColorUtils.applyAlpha(palette.getFontColor(ColorType.DARK), if (prevHovered) 255 else 200), controlSize, Fonts.LEGACYICON)

                // Play/Pause
                val playHovered = MouseUtils.isInside(mouseX, mouseY, controlsCenterX - controlSize / 2f, controlsY, controlSize, controlSize)
                val playIcon = if (isPlaying) LegacyIcon.PAUSE else LegacyIcon.PLAY
                nvg.drawCenteredText(playIcon, controlsCenterX, controlsY + 2, ColorUtils.applyAlpha(palette.getFontColor(ColorType.DARK), if (playHovered) 255 else 200), controlSize, Fonts.LEGACYICON)

                // Next
                val nextHovered = MouseUtils.isInside(mouseX, mouseY, controlsCenterX + controlSpacing - controlSize / 2f, controlsY, controlSize, controlSize)
                nvg.drawCenteredText(LegacyIcon.FORWARD, controlsCenterX + controlSpacing, controlsY + 2, ColorUtils.applyAlpha(palette.getFontColor(ColorType.DARK), if (nextHovered) 255 else 200), controlSize, Fonts.LEGACYICON)

                // Progress bar
                val progressBarY2 = controlsY + controlSize + 6
                val progressBarWidth2 = playerCardWidth - (INNER_PADDING * 2f)
                ...
                nvg.drawRoundedRect(headX, progressBarY2, progressBarWidth2 * progress2, progressBarHeight2, 1f, currentColor.color1)
                ...
                // Nada tocando
                nvg.drawText(TranslateText.NOTHING_IS_PLAYING.text, headX, spotifyY, ColorUtils.applyAlpha(palette.getFontColor(ColorType.NORMAL), 200), 8f, Fonts.REGULAR)
            }
        } else {
            // Spotify nao linkado
            nvg.drawText(TranslateText.SPOTIFY_NOT_LINKED.text, headX, spotifyY, ColorUtils.applyAlpha(Color(255, 180, 90), 220), 8f, Fonts.REGULAR)
            nvg.drawTextBox(TranslateText.SPOTIFY_LINK_DESCRIPTION.text, headX, spotifyY + 12, playerCardWidth - (INNER_PADDING * 2f), ColorUtils.applyAlpha(palette.getFontColor(ColorType.NORMAL), 180), 7f, Fonts.REGULAR)
        }

        // Discord
        val discordStartX = rightX.toInt()
        val discordStartY = playerCardY.toInt()
        val discordWidth = columnWidth.toInt()

        // bg
        nvg.drawRoundedRect(discordStartX.toFloat(), discordStartY.toFloat(), discordWidth.toFloat(), BOTTOM_CARD_HEIGHT, 8f, palette.getBackgroundColor(ColorType.DARK))
        // Discord branding
        nvg.drawRoundedRectVarying(discordStartX + discordWidth - 22F, discordStartY.toFloat(), 22F, 22F, 0F, 8F, 8F, 0F, Color(114, 137, 214))
        nvg.drawCenteredText(LegacyIcon.DISCORD, discordStartX + discordWidth - 11f, discordStartY + 4f, Color.WHITE, 14f, Fonts.LEGACYICON)
        // txt
        nvg.drawText(TranslateText.JOIN_OUR_DISCORD_SERVER.text, discordStartX + standardPadding.toFloat(), discordStartY + standardPadding.toFloat(), palette.getFontColor(ColorType.DARK), 11f, Fonts.SEMIBOLD)
        nvg.drawTextBox(TranslateText.DISCORD_DESCRIPTION.text, discordStartX + standardPadding.toFloat(), discordStartY + 26f, discordWidth - 16f, palette.getFontColor(ColorType.DARK), 8f, Fonts.REGULAR)
        // stats
        if (discStat.membersCount != -1) {
            ...
            nvg.drawRoundedGlow(discordStartX + 10f, discordStartY + BOTTOM_CARD_HEIGHT - 20, 6f, 6f, 3f, onlineColour, 7)
            nvg.drawTextGlowing(discStat.membersCount.toString() + " " + TranslateText.MEMBERS.text, discordStartX + 20f, discordStartY + BOTTOM_CARD_HEIGHT - 24, onlineColour, 4F, 8F, Fonts.REGULAR)
            nvg.drawTextGlowing(discStat.membersOnline.toString() + " " + TranslateText.ONLINE.text, discordStartX + 20f, discordStartY + BOTTOM_CARD_HEIGHT - 16, onlineColour, 4F, 8F, Fonts.REGULAR)
        }
        // join button
        nvg.drawRoundedRect(discordStartX + discordWidth - 60f, discordStartY + BOTTOM_CARD_HEIGHT - 28, 52f, 18f, 9f, Color(114, 137, 214))
        nvg.drawCenteredText(TranslateText.JOIN.text + " >", discordStartX + discordWidth - 60f + (52 / 2f), discordStartY + BOTTOM_CARD_HEIGHT - 22, Color.WHITE, 7f, Fonts.REGULAR)
    }
    ...
    override fun mouseClicked(mouseX: Int, mouseY: Int, mouseButton: Int) {
        ...
        if (MouseUtils.isInside(mouseX, mouseY, joinButtonX.toFloat(), joinButtonY.toFloat(), 52f, 18f)) {
            try {
                ...
                ShindoLogger.error(TranslateText.DISCORD_LINK_ERROR.text, e)
            }
        }
        ...
        if (musicManager != null && musicManager.isAuthorized) {
            ...
            if (mouseButton == 0) {
                ...
                if (MouseUtils.isInside(mouseX, mouseY, headX, progressBarY, progressBarWidth, 2f)) {
                    val currentTrack = musicManager.currentTrack
                    if (currentTrack != null) {
                        val trackDuration = (musicManager.endTime * 1000).toLong()
                        if (trackDuration > 0) {
