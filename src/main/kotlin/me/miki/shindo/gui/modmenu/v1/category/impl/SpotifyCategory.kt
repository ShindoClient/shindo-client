package me.miki.shindo.gui.modmenu.v1.category.impl
...
class SpotifyCategory(parent: GuiModMenu) : Category(parent, TranslateText.SPOTIFY, LegacyIcon.SPOTIFY, true, true),
    ...
    private val searchDebouncer: ScheduledExecutorService = Executors.newSingleThreadScheduledExecutor { r ->
    ...
    @Volatile private var searchResults: List<Track>? = null
    @Volatile private var searchPlaylistResults: List<PlaylistSimplified>? = null
    ...
    private fun tryOpenBrowser(uri: String) {
        try {
            ...
            Shindo.getInstance().notificationManager.post(
                TranslateText.SPOTIFY_AUTH, TranslateText.SPOTIFY_FAIL_BROWSER, NotificationType.ERROR)
    ...
    override fun drawScreen(mouseX: Int, mouseY: Int, partialTicks: Float) {
        ...
        val accentColor = instance.colorManager.getCurrentColor()
        val musicManager = instance.musicManager

        if (!isSearching.get()) checkAndUpdateSearch()
        if (showingLyrics) { lyricsScroll.onScroll(); lyricsScroll.onAnimation() }
        ...
        if (showConnectButton) {
            ...
            if (!showingLyrics) {
                nvg.drawVerticalGradientRect(getX() + 15f, getY().toFloat(), getWidth() - 30f, 12f, palette.getBackgroundColor(ColorType.NORMAL), noColour)
                nvg.drawVerticalGradientRect(getX() + 15f, getY() + getHeight() - 58f, getWidth() - 30f, 12f, noColour, palette.getBackgroundColor(ColorType.NORMAL))
            }
        }
    ...
    private fun drawConnectButton(nvg: NanoVGManager, mouseX: Int, mouseY: Int) {
        ...
        val buttonWidth = 150f; val buttonHeight = 40f
        val buttonX = centerX - (buttonWidth / 2f); val buttonY = centerY - (buttonHeight / 2f)
        ...
        val iconWidth = 16f; val spacing = 8f
    ...
    private fun drawSearchResults(nvg: NanoVGManager, palette: ColorPalette, accentColor: AccentColor, mouseX: Int, mouseY: Int) {
    ...
    private fun drawPlaceholderImage(nvg: NanoVGManager, offsetY: Float) {
        try { nvg.drawRoundedImage(PLACEHOLDER_IMAGE, getX() + 20f, getY() + offsetY + 5f, 36f, 36f, 6f) }
        catch (e: Exception) { nvg.drawRoundedRect(getX() + 20f, getY() + offsetY + 5f, 36f, 36f, 6f, Color(50, 50, 50)) }
    }
    ...
    private fun isEntryHovered(mouseX: Int, mouseY: Int, offsetY: Float): Boolean =
        MouseUtils.isInside(mouseX, mouseY, getX() + 15f, getY() + offsetY + scroll.getValue(), getWidth() - 30f, 46f)
    ...
    private fun drawPlaybackControls(nvg: NanoVGManager, palette: ColorPalette, musicManager: MusicManager) {
        val centerX = getX() + (getWidth() / 2f); val centerY = getY() + getHeight() - 32f
        ...
        nvg.drawText(if (musicManager.isPlaying()) LegacyIcon.PAUSE else LegacyIcon.PLAY, centerX - 8f, centerY, color, 16f, Fonts.LEGACYICON)
    ...
    private fun drawVolumeSlider(nvg: NanoVGManager, palette: ColorPalette, mouseX: Int, mouseY: Int, partialTicks: Float) {
        volumeSlider.setX(getX() + getWidth() - 72f); volumeSlider.setY(getY() + getHeight() - 20f)
        volumeSlider.setWidth(62f); volumeSlider.setHeight(4.5f)
        volumeSlider.draw(mouseX, mouseY, partialTicks)
        val volume = (volumeSlider.getSetting().getValueFloat() * 100).toInt()
        val icon = when { volume == 0 -> LegacyIcon.VOLUME_X; volume > 80 -> LegacyIcon.VOLUME_2; volume > 40 -> LegacyIcon.VOLUME_1; else -> LegacyIcon.VOLUME }
        nvg.drawText(icon, getX() + getWidth() - 94f, getY() + getHeight() - 26f, palette.getFontColor(ColorType.NORMAL), 16f, Fonts.LEGACYICON)
    }

    private fun drawProgressBar(nvg: NanoVGManager, accentColor: AccentColor, palette: ColorPalette) {
        if (trackDuration <= 0) return
        val y = (getY() + getHeight() - 5).toFloat(); val w = (getWidth() - 40).toFloat()
        nvg.drawRoundedRect(getX() + 20f, y, w, 2f, 1f, palette.getBackgroundColor(ColorType.NORMAL))
        nvg.drawRoundedRect(getX() + 20f, y, w * (trackPosition.toFloat() / trackDuration.toFloat()), 2f, 1f, accentColor.getInterpolateColor())
    }

    private fun drawLyricsButton(nvg: NanoVGManager, palette: ColorPalette, mouseX: Int, mouseY: Int) {
        val bx = getX() + getWidth() - 116f; val by = getY() + getHeight() - 26f
        nvg.drawText(LegacyIcon.LIST, bx, by, if (MouseUtils.isInside(mouseX, mouseY, bx, by, 16f, 16f)) palette.getFontColor(ColorType.DARK) else palette.getFontColor(ColorType.NORMAL), 16f, Fonts.LEGACYICON)
    }
    ...
    private fun drawLyricsView(nvg: NanoVGManager, palette: ColorPalette, accentColor: AccentColor, musicManager: MusicManager, mouseX: Int, mouseY: Int) {
        ...
        nvg.drawText(LegacyIcon.ARROW_LEFT, getX() + 15f, getY() + 15f, if (isBackHovered) palette.getFontColor(ColorType.DARK) else palette.getFontColor(ColorType.NORMAL), 16f, Fonts.LEGACYICON)
        if (musicManager.getCurrentTrack() != null) {
            if (MouseUtils.isInside(mouseX, mouseY, getX().toFloat(), getY().toFloat(), getWidth().toFloat(), getHeight() - 46f)) lyricsScroll.onScroll()
    ...
    private fun drawScrollableLyrics(nvg: NanoVGManager, palette: ColorPalette, accentColor: AccentColor, musicManager: MusicManager, mouseX: Int, mouseY: Int, startY: Float, currentPosition: Long) {
        ...
        for (i in allLines.indices) {
            ...
            if (text.isEmpty()) { lineHeights[i] = 30; wrappedLines[i] = emptyArray(); continue }
        ...
        ModMenuClipCoordinator.withClip(nvg = nvg, x = getX() + 15f, y = getY() + startY, width = getWidth() - 30f, height = lyricsAreaHeight) {
            var currentY = getY() + startY + yOffset
            val visibleTop = getY() + startY; val visibleBottom = visibleTop + lyricsAreaHeight
            for (i in allLines.indices) {
                if (currentY + lineHeights[i] < visibleTop || currentY > visibleBottom) { currentY += lineHeights[i]; continue }
                ...
                val lineColor: Color; val fontSize: Float
                if (isCurrentLine) {
                    lineColor = accentColor.getInterpolateColor(); fontSize = 14f
                    nvg.drawRoundedRect(getX() + 20f, currentY, getWidth() - 40f, lineHeights[i].toFloat(), 4f, Color(accentColor.getColor1().red, accentColor.getColor1().green, accentColor.getColor1().blue, 30))
                } else if (isHovered) { lineColor = palette.getFontColor(ColorType.DARK); fontSize = 12f
                } else { lineColor = palette.getFontColor(ColorType.NORMAL); fontSize = 12f }
                ...
                if (wrapped.isNotEmpty()) {
                    ...
                    for (line in wrapped) {
                        ...
                        if (isCurrentLine) nvg.drawTextGlowing(line, tx, ty, lineColor, 8f, fontSize, Fonts.MEDIUM)
                        else nvg.drawText(line, tx, ty, lineColor, fontSize, Fonts.MEDIUM)
                        wrapOffset += fontSize
                    }
                }
        ...
        if (lyricsScroll.getValue() < 0) nvg.drawVerticalGradientRect(getX() + 15f, getY() + startY, getWidth() - 30f, 12f, palette.getBackgroundColor(ColorType.NORMAL), noColour)
        if (-lyricsScroll.getValue() < maxScroll) nvg.drawVerticalGradientRect(getX() + 15f, getY() + startY + lyricsAreaHeight - 12f, getWidth() - 30f, 12f, noColour, palette.getBackgroundColor(ColorType.NORMAL))
    }

    private fun wrapText(nvg: NanoVGManager, text: String?, fontSize: Float, font: Font, maxWidth: Float): Array<String> {
        if (text == null || text.isEmpty()) return emptyArray()
        if (nvg.getTextWidth(text, fontSize, font) <= maxWidth) return arrayOf(text)
        val lines = ArrayList<String>(); var currentLine = StringBuilder()
        for (word in text.split(" ")) {
            val testLine = if (currentLine.isNotEmpty()) "$currentLine $word" else word
            if (nvg.getTextWidth(testLine, fontSize, font) <= maxWidth) { if (currentLine.isNotEmpty()) currentLine.append(" "); currentLine.append(word) }
            else {
                if (currentLine.isNotEmpty()) { lines.add(currentLine.toString()); currentLine = StringBuilder() }
                if (nvg.getTextWidth(word, fontSize, font) > maxWidth) {
                    var partial = StringBuilder()
                    for (c in word) { if (nvg.getTextWidth("$partial$c", fontSize, font) <= maxWidth) partial.append(c) else { lines.add(partial.toString()); partial = StringBuilder().append(c) } }
    ...
    override fun mouseClicked(mouseX: Int, mouseY: Int, mouseButton: Int) {
        ...
        if (openDownloader) { handleDownloaderClick(mouseX, mouseY, mouseButton); return }
        ...
        if (!showingLyrics && mouseButton == 0) {
            ...
            if (MouseUtils.isInside(mouseX, mouseY, bx, by, 16f, 16f)) {
                showingLyrics = true; lyricsScroll.resetAll()
                val mm = Shindo.getInstance().musicManager
                if (mm.getCurrentTrack() != null) mm.getLyricsManager().fetchLyrics(mm.getCurrentTrack())
                return
            }
        }

        if (mouseButton == 0 && mouseY >= getY() + getHeight() - 46) { handleControlBarClick(mouseX, mouseY); return }

        if (showingLyrics) {
            if (MouseUtils.isInside(mouseX, mouseY, getX() + 15f, getY() + 15f, 16f, 16f) && mouseButton == 0) { showingLyrics = false; return }
        ...
        if (mouseButton == 0 && searchResults != null) handleTrackClick(mouseX, mouseY)
        else if (mouseButton == 0 && userPlaylists != null) handlePlaylistClick(mouseX, mouseY)
    }
    ...
    private fun handleDownloaderClick(mouseX: Int, mouseY: Int, mouseButton: Int) {
        ...
        if (MouseUtils.isInside(mouseX, mouseY, getX() + getWidth() - 34f, getY() + getHeight() - 80f, 18f, 18f) && mouseButton == 0) {
            openDownloader = false; Shindo.getInstance().musicManager.play(textBox.getText()); return
        }
        if (!MouseUtils.isInside(mouseX, mouseY, getX() + getWidth() - 175f, getY() + getHeight() - 86f, 165f, 30f)) openDownloader = false
    }

    private fun handleControlBarClick(mouseX: Int, mouseY: Int) {
        val mm = Shindo.getInstance().musicManager
        val cx = getX() + (getWidth() / 2f); val cy = getY() + getHeight() - 32f
        if (MouseUtils.isInside(mouseX, mouseY, cx - 32f, cy, 16f, 16f)) { mm.previousTrack(); return }
        if (MouseUtils.isInside(mouseX, mouseY, cx - 8f, cy, 16f, 16f)) { if (mm.isPlaying()) mm.pause() else mm.resume(); return }
        if (MouseUtils.isInside(mouseX, mouseY, cx + 16f, cy, 16f, 16f)) { mm.nextTrack(); return }
        if (MouseUtils.isInside(mouseX, mouseY, getX() + getWidth() - 72f, getY() + getHeight() - 22f, 62f, 8f)) { volumeSlider.mouseClicked(mouseX, mouseY, 0); return }
    ...
    private fun handleTrackClick(mouseX: Int, mouseY: Int) {
        ...
        searchResults?.forEach { track ->
            if (MouseUtils.isInside(mouseX, mouseY, getX() + 15f, getY() + offsetY, getWidth() - 30f, 46f)) {
                if (MouseUtils.isInside(mouseX, mouseY, getX() + getWidth() - 60f, getY() + offsetY + 15f, 16f, 16f)) addToQueue(track)
                else Shindo.getInstance().musicManager.play(track.uri)
                return
            }
            offsetY += 56f
        }
        searchPlaylistResults?.forEach { playlist ->
            ...
            if (MouseUtils.isInside(mouseX, mouseY, getX() + 15f, getY() + offsetY, getWidth() - 30f, 46f)) {
                try { Shindo.getInstance().musicManager.playPlaylist(playlist.uri) }
                catch (e: Exception) { Shindo.getInstance().notificationManager.post(TranslateText.MUSIC, TranslateText.SPOTIFY_FAILED_TO_PLAY_PLAYLIST, NotificationType.ERROR) }
                return
            }
            offsetY += 56f
        }
    ...
    private fun handlePlaylistClick(mouseX: Int, mouseY: Int) {
        ...
        for (playlist in playlists) {
            if (playlist.uri == null) { offsetY += 56f; continue }
            if (MouseUtils.isInside(mouseX, mouseY, getX() + 15f, getY() + offsetY, getWidth() - 30f, 46f)) {
                try { Shindo.getInstance().musicManager.playPlaylist(playlist.uri) }
                catch (e: Exception) { Shindo.getInstance().notificationManager.post(TranslateText.MUSIC, TranslateText.SPOTIFY_FAILED_TO_PLAY_PLAYLIST, NotificationType.ERROR) }
                break
    ...
    private fun addToQueue(track: Track) {
        Shindo.getInstance().musicManager.addToQueue(track.uri)
            ...
            .exceptionally { Shindo.getInstance().notificationManager.post(TranslateText.MUSIC, TranslateText.SPOTIFY_FAILED_TO_ADD_TO_QUEUE, NotificationType.ERROR); null }
    }

    override fun mouseReleased(mouseX: Int, mouseY: Int, mouseButton: Int) {
        if (!MouseUtils.isInside(mouseX, mouseY, getX().toFloat(), getY().toFloat(), getWidth().toFloat(), getHeight().toFloat())) return
        if (!showConnectButton) { volumeSlider.mouseReleased(mouseX, mouseY, mouseButton); updateVolume() }
    }
    ...
    private fun updateVolume() {
        ...
        if (now - lastVolumeChangeTime > VOLUME_CHANGE_DELAY) {
            ...
            Shindo.getInstance().musicManager.setVolume((volumeSlider.getSetting().getValueFloat() * 100).toInt())
        }
    }
    ...
    override fun keyTyped(typedChar: Char, keyCode: Int) {
        ...
        if (keyCode == Keyboard.KEY_SPACE && !searchFocused) { if (mm.isPlaying()) mm.pause() else mm.resume() }
        if (keyCode == Keyboard.KEY_UP) { val v =
            min(100, (volumeSlider.getSetting().getValueFloat() * 100).toInt() + 5); volumeSlider.getSetting().setValue(v / 100.0); mm.setVolume(v); lastVolumeChangeTime = System.currentTimeMillis() }
        else if (keyCode == Keyboard.KEY_DOWN) { val v =
            max(0, (volumeSlider.getSetting().getValueFloat() * 100).toInt() - 5); volumeSlider.getSetting().setValue(v / 100.0); mm.setVolume(v); lastVolumeChangeTime = System.currentTimeMillis() }
        if (keyCode == Keyboard.KEY_RIGHT) mm.seekToPosition(min(trackPosition + 10000, trackDuration))
        else if (keyCode == Keyboard.KEY_LEFT) mm.seekToPosition(max(trackPosition - 10000, 0))
    ...
    override fun onTrackInfoUpdated(position: Long, duration: Long) {
        trackPosition = position; trackDuration = duration
        val mm = Shindo.getInstance().musicManager
        ...
        if (currentTrack != null) {
            if (currentTrack.id != currentTrackId) {
                ...
                if (showingLyrics) { mm.getLyricsManager().fetchLyrics(currentTrack); lyricsScroll.resetAll() }
            }
        } else currentTrackId = null
    }
    ...
    private fun checkAndUpdateSearch() {
        ...
        if (query != lastSearchQuery) { scheduleSearch(query); lastSearchQuery = query }
    }

    private fun scheduleSearch(query: String) {
        if (query.isEmpty()) { searchResults = null; searchPlaylistResults = null; return }
