...
class SpotifyCategory(parent: GuiModMenu) : Category(parent, TranslateText.SPOTIFY, LegacyIcon.SPOTIFY, true, true),
    ...
    private fun handleDownloaderClick(mouseX: Int, mouseY: Int, mouseButton: Int) {
        ...
        if (MouseUtils.isInside(mouseX, mouseY, getX() + getWidth() - 34f, getY() + getHeight() - 80f, 18f, 18f) && mouseButton == 0) {
            openDownloader = false; Shindo.getInstance().getMusicManager().play(textBox.getText()); return
        }
    ...
    private fun handleTrackClick(mouseX: Int, mouseY: Int) {
        ...
        searchResults?.forEach { track ->
            if (MouseUtils.isInside(mouseX, mouseY, getX() + 15f, getY() + offsetY, getWidth() - 30f, 46f)) {
                ...
                else Shindo.getInstance().getMusicManager().play(track.uri)
                return
            }
            offsetY += 56f
        }
        searchPlaylistResults?.forEach { playlist ->
            ...
            if (MouseUtils.isInside(mouseX, mouseY, getX() + 15f, getY() + offsetY, getWidth() - 30f, 46f)) {
                try { Shindo.getInstance().getMusicManager().playPlaylist(playlist.uri) }
                catch (e: Exception) { Shindo.getInstance().getNotificationManager().post(TranslateText.MUSIC, TranslateText.SPOTIFY_FAILED_TO_PLAY_PLAYLIST, NotificationType.ERROR) }
                return
            }
            offsetY += 56f
        }
    ...
    private fun addToQueue(track: Track) {
        Shindo.getInstance().getMusicManager().addToQueue(track.uri)
            ...
            .exceptionally { Shindo.getInstance().getNotificationManager().post(TranslateText.MUSIC, TranslateText.SPOTIFY_FAILED_TO_ADD_TO_QUEUE, NotificationType.ERROR); null }
    }
