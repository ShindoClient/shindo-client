...
object CategoryChipRenderer {
    ...
    fun drawChip(
        ...
        val background = ColorUtils.applyAlpha(base, if (hovered || active) 235 else 205)

        if (active) {
            val start = ColorUtils.applyAlpha(accent.color1, 210)
            val end = ColorUtils.applyAlpha(accent.color2, 210)
