...
class SpotifyCategory(parent: GuiModMenu) : Category(parent, TranslateText.SPOTIFY, LegacyIcon.SPOTIFY, true, true),
    ...
    override fun keyTyped(typedChar: Char, keyCode: Int) {
        ...
        if (keyCode == Keyboard.KEY_UP) { val v = kotlin.math.min(100, (volumeSlider.getSetting().getValueFloat() * 100).toInt() + 5); volumeSlider.getSetting().setValue(v / 100.0); mm.setVolume(v); lastVolumeChangeTime = System.currentTimeMillis() }
        else if (keyCode == Keyboard.KEY_DOWN) { val v = kotlin.math.max(0, (volumeSlider.getSetting().getValueFloat() * 100).toInt() - 5); volumeSlider.getSetting().setValue(v / 100.0); mm.setVolume(v); lastVolumeChangeTime = System.currentTimeMillis() }
        if (keyCode == Keyboard.KEY_RIGHT) mm.seekToPosition(kotlin.math.min(trackPosition + 10000, trackDuration))
        else if (keyCode == Keyboard.KEY_LEFT) mm.seekToPosition(kotlin.math.max(trackPosition - 10000, 0))
