...
class ShopScene(parent: GuiShindoMainMenu) : MainMenuScene(parent) {
    ...
    private fun drawNanoVG(mouseX: Int, mouseY: Int, sr: ScaledResolution, instance: Shindo, nvg: NanoVGManager?) {
        ...
        nvg.drawCenteredText(TranslateText.PREMIUM.text, acX + 20f + (82 / 2f), acY + 58f, Color.WHITE, 12f, Fonts.MEDIUM)
        nvg.drawCenteredText("20$ / " + TranslateText.LIFETIME.text, acX + 20f + (82 / 2f), acY + 71f, Color.WHITE, 8f, Fonts.REGULAR)
        nvg.drawRect(acX + 20f, acY + 80f, 82f, 1f, Color.WHITE)

        nvg.drawRoundedRect(acX + 25f, acY + 153f, 72f, 20f, 6f, controlColor)
        nvg.drawCenteredText(info.text, acX + 25f + (72 / 2f), acY + 159f, Color.WHITE, 10f, Fonts.MEDIUM)

        for (t in goldFeatures) {
            nvg.drawText(LegacyIcon.CHECK_CIRCLE, acX + 25f, acY + 87f + offsetY, Color.WHITE, 9f, Fonts.LEGACYICON)
            nvg.drawText(t.text, acX + 36f, acY + 88f + offsetY, Color.WHITE, 8f, Fonts.REGULAR)

            offsetY += 12
        }

        nvg.drawCenteredText(TranslateText.SOON.text, acX + acWidth - 82f - 20f + (82 / 2f), acY + 58f, Color.WHITE, 12f, Fonts.MEDIUM)
        nvg.drawCenteredText("?? / " + TranslateText.MONTH.text, acX + acWidth - 82f - 20f + (82 / 2f), acY + 71f, Color.WHITE, 8f, Fonts.REGULAR)
        ...
        nvg.drawCenteredText(TranslateText.SOON.text, acX + acWidth - 82f - 15f + (72 / 2f), acY + 159f, Color.WHITE, 10f, Fonts.MEDIUM)
    }
