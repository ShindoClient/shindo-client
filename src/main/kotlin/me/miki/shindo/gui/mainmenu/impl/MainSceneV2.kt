...
class MainSceneV2(parent: GuiShindoMainMenu) : MainMenuScene(parent) {
    ...
    private fun drawNanoVG(instance: Shindo, nvg: NanoVGManager, mouseX: Int, mouseY: Int, partialTicks: Float) {
        ...
        if (confirmAnim.value > 0.01f) {
            drawConfirmExit(nvg, sw, sh, mouseX, mouseY, anim * confirmAnim.value)
        }
        ...
        nvg.drawText(
            "Shindo " + Shindo.getInstance().version, 8, sh - 12,
            Color(255, 255, 255, (anim * 100).toInt()), 9, Fonts.REGULAR
        )
    }
