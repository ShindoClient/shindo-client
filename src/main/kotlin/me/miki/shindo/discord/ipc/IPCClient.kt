...
class IPCClient(private val clientId: Long) : Closeable {
    ...
    fun sendRichPresence(presence: RichPresence?, callback: Callback?) {
        if (isConnected(true)) return

        LOGGER.debug("Sending RichPresence to discord: ${presence?.toJson()?.toString()}")

        val argsObject = JsonObject().apply {
            addProperty("pid", pid)
    ...
    fun subscribe(sub: Event, callback: Callback?) {
        if (isConnected(true)) return
        ...
        readThread = Thread({
            try {
                ...
                while (pipe?.read().also { p = it!! }!!.op != OpCode.CLOSE) {
                    ...
                    when (event) {
                        Event.NULL -> if (nonce != null && callbacks.containsKey(nonce)) {
                        ...
                        Event.ACTIVITY_SPECTATE -> LOGGER.debug("Reading thread received a 'spectate' event.")
                        Event.ACTIVITY_JOIN_REQUEST -> LOGGER.debug("Reading thread received a 'join request' event.")
                    ...
                    if (listener != null && json.has("cmd") && json.get("cmd").asString == "DISPATCH") {
                        try {
                            ...
                            when (Event.of(json.get("evt").asString)) {
                                Event.ACTIVITY_JOIN -> listener?.onActivityJoin(this, data.get("secret").asString)
    ...
    enum class Event(val isSubscribable: Boolean) {
        NULL(false),
        READY(false),
        ERROR(false),
        ACTIVITY_JOIN(true),
