...
open class ComboSetting : Setting {
    ...
    constructor(text: TranslateText, parent: ConfigOwner, defaultOption: TranslateText, options: List<Option>) : super(text, parent) {
        this.options = ArrayList(options)
        this.option = getOptionByKey(defaultOption.key)
        this.defaultOption = this.option
    }

    constructor(text: TranslateText, parent: ConfigOwner, defaultOptionKey: String, options: List<Option>) : super(text, parent) {
        this.options = ArrayList(options)
    ...
    constructor(name: String, parent: ConfigOwner, defaultOptionKey: String, options: List<Option>) : super(name, parent) {
        this.options = ArrayList(options)
        this.option = getOptionByKey(defaultOptionKey)
        if (this.option == null && this.options.isNotEmpty()) {
            this.option = this.options[0]
    ...
    fun getOptionByKey(key: String): Option? {
        for (op in options) {
