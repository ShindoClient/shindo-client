...
data class Vector3D(val x: Double, val y: Double, val z: Double) {
    ...
    private fun dotProduct(other: Vector3D): Double = x * other.x + y * other.y + z * other.z

    fun getXZAngleDiffWithVector(other: Vector3D): Double {
        val den = sqrt((x * x + z * z) * (other.x * other.x + other.z * other.z))
    ...
    private fun getProjectionInXZPlane(): Vector2D = Vector2D(x, z)

    fun normInXZPlane(): Double = getProjectionInXZPlane().norm()

    fun getAngleWithVector(other: Vector3D): Double {
        val den = sqrt((x * x + y * y + z * z) * (other.x * other.x + other.y * other.y + other.z * other.z))
        if (den < 1.0000000116860974E-7) return 0.0
