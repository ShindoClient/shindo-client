...
/**
 * Classe base abstrata para checks de anti-cheat
 * Fornece métodos utilitários e integração com NoCheaters
 */
abstract class Check : ICheck {
    ...
    /**
     * Verifica se este check está habilitado
     * Verifica primeiro se o addon está habilitado antes de acessar as settings
     */
    protected fun isCheckEnabled(): Boolean {

        val addon = HackerDetectorAddon.instance
        // Verifica se o addon está habilitado (isToggled) antes de acessar settings
        if (!addon.isToggled()) return false

        
        // Verifica se o check específico está habilitado
        return try {
            when (getCheatName()) {
                ...
                "Fly" -> addon.enableFlyCheck
                "Speed" -> addon.enableSpeedCheck
                "Jesus" -> addon.enableJesusCheck
                "NoFall" -> addon.enableNoFallCheck
                else -> true
            }
        } catch (e: Exception) {
            // Settings podem não estar disponíveis durante inicialização
            false
        }
    }
    ...
    /**
     * Verifica o nível de violação e executa ações apropriadas
     */
    ...
    private fun playFlagSound() {
        ...
        try {
            if (addon.soundWhenFlaggingSetting && System.currentTimeMillis() - getLastSoundTime() > 250) {
                try {
                    // Tenta tocar som customizado, se não existir usa som padrão do Minecraft
                    try {
                        me.miki.shindo.utils.Sound.play("shindo/sounds/notification.ogg", false)
                    } catch (_: Exception) {
                        // Fallback para som padrão do Minecraft
            ...
            // Settings podem não estar disponíveis
        }
    }
                        ...
                        ChatComponentText(" ${EnumChatFormatting.GREEN}[WDR]").apply {
                            chatStyle = ChatStyle().apply {
                                chatClickEvent = ClickEvent(
                                    ...
                                    "/wdr $playername ${getCheatName().lowercase()}"
                                )
                            }
                        }
                    )
    ...
    private fun addToReportList(player: EntityPlayer) {
        ...
        try {
            ...
            val cheat = "${getCheatName().lowercase()}[H]"
    ...
    /**
     * Retorna a velocidade base para um jogador correndo sem pular
     */
    ...
    /**
     * Retorna a quantidade de ticks para um jogador quebrar um bloco
     */
    protected fun getTimeToHarvestBlock(
        ...
        // Calcula blockStrength manualmente (alternativa ao ForgeHooks que não existe em 1.8.9)
    ...
    protected fun isInsideHitbox(playerX: Double, playerY: Double, playerZ: Double, x: Double, y: Double, z: Double): Boolean {
