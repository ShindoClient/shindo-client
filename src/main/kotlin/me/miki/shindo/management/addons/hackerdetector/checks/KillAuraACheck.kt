...
class KillAuraACheck(private val recentPlacedBlocks: TickingBlockMap) : Check() {
    ...
    override fun check(player: EntityPlayer, data: PlayerDataSamples): Boolean {
        ...
        val lookEndPos =
            attackerEyePos.addVector(lookVect.xCoord * maxReach, lookVect.yCoord * maxReach, lookVect.zCoord * maxReach)
        ...
        for (i in 0..maxSteps) {
            ...
            if (xpos != blockXpos || ypos != blockYpos || zpos != blockZpos) {
                ...
                canHitThroughBlock =
                    !block.isFullBlock || !block.canCollideCheck(iblockstate, false) || recentPlacedBlocks.contains(pos)
                blockXpos = xpos
                blockYpos = ypos
                blockZpos = zpos
            }
            ...
            if (nearbyPlayers.isNotEmpty() && nearbyPlayers.size < 15) {
                for (j in 0..iterMax) {
                    ...
                    for (eData in nearbyPlayers) {
                        if (isInsideHitbox(
                                eData.posXList.get(i),
                                eData.posYList.get(i),
                                eData.posZList.get(i),
                                dx,
                                dy,
                                dz
                            )
        ...
        if (b + p > 0) {
            val vlb = 15.coerceAtMost(b)
            val vlp = 8.coerceAtMost(p)
            data.killAuraAVL.add(15.coerceAtMost(vlb + (if (nearbyPlayers.size > 8) vlp / 2 else vlp)) * 25)
