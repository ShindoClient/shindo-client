...
object CompStyleResolver {
    ...
    fun resolveSurfaceBackground(variant: CompSurfaceVariant, palette: ColorPalette, accent: AccentColor): Color {
        val base = when (variant) {
            ...
            CompSurfaceVariant.PANEL -> ColorUtils.applyAlpha(palette.getBackgroundColor(ColorType.DARK), 238)
            CompSurfaceVariant.CARD -> ColorUtils.applyAlpha(palette.getBackgroundColor(ColorType.MID), 224)
    ...
    fun resolveSurfaceBorder(variant: CompSurfaceVariant, palette: ColorPalette, accent: AccentColor): Color {
        val base = when (variant) {
            CompSurfaceVariant.CANVAS -> ColorUtils.applyAlpha(palette.getFontColor(ColorType.NORMAL), 28)
            CompSurfaceVariant.PANEL -> ColorUtils.applyAlpha(palette.getFontColor(ColorType.NORMAL), 34)
            CompSurfaceVariant.CARD -> ColorUtils.applyAlpha(accent.getColor1(), 76)
            CompSurfaceVariant.OVERLAY -> ColorUtils.applyAlpha(accent.getColor2(), 84)
            CompSurfaceVariant.TRANSPARENT -> Color(0, 0, 0, 0)
        }
        return VisualPresetModificationManager.applySurfaceBorder(activePreset(), variant, base, palette, accent)
    }

    fun resolveControlBase(variant: CompControlVariant, palette: ColorPalette, accent: AccentColor): Color {
        val base = when (variant) {
            CompControlVariant.PRIMARY -> ColorUtils.applyAlpha(accent.getColor1(), 190)
    ...
    fun resolveControlHover(variant: CompControlVariant, palette: ColorPalette, accent: AccentColor): Color {
        return when (variant) {
            CompControlVariant.PRIMARY -> ColorUtils.applyAlpha(accent.getColor2(), 225)
            CompControlVariant.SECONDARY -> ColorUtils.applyAlpha(accent.getColor1(), 165)
            CompControlVariant.GHOST -> ColorUtils.applyAlpha(accent.getColor1(), 145)
