...
class CompProxyCard : Comp() {
    ...
    override fun draw(mouseX: Int, mouseY: Int, partialTicks: Float) {
        ...
        togglePressAnimation.setAnimation(
            if (togglePressAnimation.value > 0.08f) togglePressAnimation.value * 0.82f else 0.0f,
            16.0
        )
        toggleAnimation.setAnimation(if (active) 1.0f else 0.0f, 16.0)

        val toggleProgress = toggleAnimation.value
        val hoverProgress = toggleHoverAnimation.value
        val pressProgress = togglePressAnimation.value
    ...
    override fun mouseClicked(mouseX: Int, mouseY: Int, mouseButton: Int) {
        ...
        if (MouseUtils.isInside(mouseX, mouseY, toggleX, toggleY, TOGGLE_WIDTH, TOGGLE_HEIGHT)) {
            togglePressAnimation.value = 1.0f
