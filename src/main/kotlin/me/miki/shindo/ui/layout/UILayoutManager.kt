...
class UILayoutManager {
    ...
    fun getTypes(area: UILayoutArea): List<UILayoutType> = when (area) {
    ...
    fun getSelectedType(area: UILayoutArea): UILayoutType? = when (area) {
        UILayoutArea.SETTINGS -> when (InternalSettingsMod.instance.settingsLayoutMode) {
        ...
        UILayoutArea.VISUAL -> when (InternalSettingsMod.instance.getVisualPreset()) {
            ...
            InternalSettingsMod.VisualPreset.CLASSIC -> UILayoutType.VISUAL_MODERN
        }
    }
    ...
    private val visualTypes = listOf(
