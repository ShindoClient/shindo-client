...
/**
 * Renderizador customizado para o menu de configurações de vídeo do Minecraft (GuiVideoSettings).
 * Aplica o estilo visual do Shindo Client mantendo toda a funcionalidade original.
 * Suporta tanto o menu padrão quanto o do OptiFine.
 */
object MinecraftVideoSettingsScreenRenderer {
    
    /**
     * Renderiza o fundo e layout do menu de configurações de vídeo com o estilo do Shindo Client.
     */
    fun renderVideoSettingsScreen(screen: GuiScreen, mouseX: Int, mouseY: Int, partialTicks: Float) {
        ...
        val title = net.minecraft.client.resources.I18n.format("options.video")
    ...
    /**
     * Verifica se a tela é do OptiFine.
     */
    fun isOptiFineScreen(screen: GuiScreen): Boolean {
        // Verifica se a classe contém "OF" (OptiFine) no nome
        return screen.javaClass.simpleName.contains("OF", ignoreCase = true) ||
               screen.javaClass.name.contains("optifine", ignoreCase = true)
    }
}
