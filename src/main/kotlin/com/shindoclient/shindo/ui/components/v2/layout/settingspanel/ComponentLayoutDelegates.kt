package com.shindoclient.shindo.ui.components.v2.layout.settingspanel
...
import com.shindoclient.shindo.ui.components.v2.inputs.CompCellGrid
import com.shindoclient.shindo.ui.components.v2.inputs.CompColorPicker
import com.shindoclient.shindo.ui.components.v2.inputs.CompComboBox
import com.shindoclient.shindo.ui.components.v2.inputs.CompImageSelect
import com.shindoclient.shindo.ui.components.v2.inputs.CompKeybind
import com.shindoclient.shindo.ui.components.v2.inputs.CompModTextBox
import com.shindoclient.shindo.ui.components.v2.inputs.CompSlider
import com.shindoclient.shindo.ui.components.v2.inputs.CompSoundSelect
...
data class ComponentPlacement(
    val controlLeft: Float = Float.NaN,
)

interface ComponentLayoutDelegate {
    fun canHandle(comp: Component): Boolean
    ...
    fun place(
        comp: Component,
        context: ComponentLayoutContext,
    ): ComponentPlacement

    fun preferFullWidth(comp: Component): Boolean = false
}

class ComponentLayoutRegistry(
    private val delegates: List<ComponentLayoutDelegate>,
) {
    private val fallback =
        object : ComponentLayoutDelegate {
            override fun canHandle(comp: Component): Boolean = true

            override fun targetHeight(
                comp: Component,
                narrow: Boolean,
                style: SettingsPanelStyle,
            ): Float = if (narrow) style.minRowHeightNarrow else style.minRowHeightDefault
    ...
    fun resolve(comp: Component): ComponentLayoutDelegate {
...
private class ToggleDelegate : ComponentLayoutDelegate {
    override fun canHandle(comp: Component): Boolean = comp is CompToggleButton

    override fun targetHeight(
        comp: Component,
        narrow: Boolean,
        style: SettingsPanelStyle,
    ): Float = if (narrow) 56f else 42f
...
private class SliderDelegate : ComponentLayoutDelegate {
    override fun canHandle(comp: Component): Boolean = comp is CompSlider

    override fun targetHeight(
        comp: Component,
        narrow: Boolean,
        style: SettingsPanelStyle,
    ): Float = if (narrow) 70f else 54f
...
private class ComboDelegate : ComponentLayoutDelegate {
    override fun canHandle(comp: Component): Boolean = comp is CompComboBox

    override fun targetHeight(
        comp: Component,
        narrow: Boolean,
        style: SettingsPanelStyle,
    ): Float = if (narrow) 64f else 46f
...
private class KeybindDelegate : ComponentLayoutDelegate {
    override fun canHandle(comp: Component): Boolean = comp is CompKeybind

    override fun targetHeight(
        comp: Component,
        narrow: Boolean,
        style: SettingsPanelStyle,
    ): Float = if (narrow) 58f else 40f
...
private class TextBoxDelegate : ComponentLayoutDelegate {
    override fun canHandle(comp: Component): Boolean = comp is CompModTextBox
    ...
    override fun place(
        ...
        val x =
            if (context.narrow) {
                context.x + context.style.componentPadding + 2f
            } else {
                right - width -
                    context.style.componentPadding
            }
        val y = if (context.narrow) context.y + 30f else context.y + context.style.componentPadding + 2f
        comp.setWidth(width)
        comp.setHeight(18f)
...
private class ImageDelegate : ComponentLayoutDelegate {
    override fun canHandle(comp: Component): Boolean = comp is CompImageSelect
...
private class SoundDelegate : ComponentLayoutDelegate {
    override fun canHandle(comp: Component): Boolean = comp is CompSoundSelect
...
private class ColorPickerDelegate : ComponentLayoutDelegate {
    override fun canHandle(comp: Component): Boolean = comp is CompColorPicker
...
private class CellGridDelegate : ComponentLayoutDelegate {
    override fun canHandle(comp: Component): Boolean = comp is CompCellGrid
    ...
    override fun preferFullWidth(comp: Component): Boolean = true
}
